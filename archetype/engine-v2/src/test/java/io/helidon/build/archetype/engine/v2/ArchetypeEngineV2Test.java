...
class ArchetypeEngineV2Test {
    ...
    void testExternalDefaultZip() throws IOException {
        Path outputDir = e2eZip("testExternalDefaultZip", null,
                Map.of("theme", "colors",
                        "theme.base", "rainbow"),
    ...
    void testOutputPropsFile() throws IOException {
        Path outputDir = e2eZip("testOutputPropsFile", "output.properties",
                Map.of("theme", "shapes",
                        "theme.base", "custom"),
                Map.of());
    ...
    private void assert2dShapesReadme(Path outputDir) throws IOException {
        ...
        assertThat(readme, is("# my-project\n"
                              + "\n"
                              + "This the README\n"
                              + "\n"
    ...
    private void assert2dShapes(Path outputDir) throws IOException {
        ...
        assertThat(readFile(shapesFiles), is("Circle\n"
                                             + "Triangle\n"
                                             + "Rectangle\n"));
    }
    ...
    private void assertCustomDefaultShapes(Path outputDir) throws IOException {
        ...
        assertThat(readFile(shapesFile), is("Circle\n"
                                            + "Triangle\n"));
    }
    ...
    private void assertCustomShapes(Path outputDir) throws IOException {
        Path shapesFile = outputDir.resolve("shapes.txt");
        assertThat(Files.exists(shapesFile), is(true));
        assertThat(readFile(shapesFile), is("Arrow\n"
                                            + "Donut\n"));
    }
    ...
    private void assert2dModernStyle(Path outputDir) throws IOException {
        ...
        assertThat(readFile(modernFile), is("Modern style.\n"
                                            + "\n"
                                            + "Notes:\n"
    ...
    static void assertCustomColors(Path outputDir) throws IOException {
        Path colorsFile = outputDir.resolve("colors.txt");
        assertThat(Files.exists(colorsFile), is(true));
        assertThat(readFile(colorsFile), is("Cyan\n"
                                            + "Khaki\n"));
    }

    static void assertDefaultCustomColors(Path outputDir) throws IOException {
        Path colorsFile = outputDir.resolve("colors.txt");
        assertThat(Files.exists(colorsFile), is(true));
        assertThat(readFile(colorsFile), is("Red\n"
                                            + "Green\n"
                                            + "Blue\n"));
    }

    static void assertRainbowColors(Path outputDir) throws IOException {
        Path colorsFile = outputDir.resolve("colors.txt");
        assertThat(Files.exists(colorsFile), is(true));
    ...
    static void assertCustomColorsModernStyle(Path outputDir) throws IOException {
        Path modernFile = outputDir.resolve("modern.txt");
        assertThat(Files.exists(modernFile), is(true));
    ...
    static void assertRainbowModernStyle(Path outputDir) throws IOException {
        Path modernFile = outputDir.resolve("modern.txt");
        assertThat(Files.exists(modernFile), is(true));
        assertThat(readFile(modernFile), is(""
                                            + "Modern style.\n"
    ...
    Path e2eZip(String name, Map<String, String> externalValues) throws IOException {
        return e2eZip(name, null, externalValues, Map.of());
    }

    Path e2eZip(String name,
    ...
    Path e2eDir(String name, Map<String, String> externalValues, Map<String, String> externalDefaults) {
        Path targetDir = targetDir(this.getClass());
        Path sourceDir = targetDir.resolve("test-classes/e2e");
        Path testOutputDir = targetDir.resolve("engine-ut");
