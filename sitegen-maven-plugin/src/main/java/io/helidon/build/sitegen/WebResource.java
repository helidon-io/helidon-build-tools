...
public class WebResource implements Model {
    ...
    public static class Builder extends AbstractBuilder<WebResource> {
        ...
        public Builder config(Config node){
            node.get(PATH_PROP).asString().ifPresent(it -> put(PATH_PROP, it));
            node.get(HREF_PROP).asString().ifPresent(it -> put(HREF_PROP, it));
            node.get(TYPE_PROP).asString().ifPresent(it -> put(TYPE_PROP, it));
