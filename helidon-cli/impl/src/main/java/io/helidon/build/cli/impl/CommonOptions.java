...
final class CommonOptions {
    ...
    CommonOptions(@Option.Flag(name = VERBOSE_FLAG_NAME, description = VERBOSE_FLAG_DESCRIPTION, visible = false) boolean verbose,
        ...
        this.debug = debug;
        this.plain = plain || setUserConfig().richTextDisabled();
        this.projectDir = projectDir != null ? projectDir.toPath().toAbsolutePath() : WORKING_DIR;
        this.metadataUrl = (metadataUrl == null || metadataUrl.isBlank()) ? setUserConfig().updateUrl() : metadataUrl;
    ...
    Metadata metadata() {
        if (metadata == null) {
            UserConfig config = setUserConfig();
            if (resetCache) {
                try {
